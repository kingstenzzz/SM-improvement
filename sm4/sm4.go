...
//func p1(a uint32) uint32 {
//	//fmt.Println("p1p1p1p1p1p1p1")
//	var aArr [4]byte
//	var bArr [4]byte
...
func permuteInitialBlock1(b []uint32, block []byte) {
...
func permuteInitialBlock2(b []uint32, block []byte) {
...
func permuteFinalBlock1(b []byte, block []uint32) {
...
func cryptBlock(subkeys []uint32, b []uint32, r []byte, dst, src []byte, decrypt bool) {
	...
	if decrypt {
		...
		for i = 0; i < 32; i++ {
			x[i+4] = x[i] ^ t(x[i+1]^x[i+2]^x[i+3]^subkeys[31-i])

		}

		for i = 32 + 3; i >= 32; i-- {
			binary.BigEndian.PutUint32(dst[4*(32+3-i):], x[i])
		}*/
	} else {
		...
		for i = 0; i < 32; i++ {
			x[i+4] = x[i] ^ t(x[i+1]^x[i+2]^x[i+3]^subkeys[i])
		}
...
func Sm4Cbc(key []byte, in []byte, mode bool) (out []byte, err error) {
	...
	//fmt.Printf("len(inData)=%v",len(inData))
